...
public class HistorialOrdenServiceImpl implements HistorialOrdenService{
	...
	public Response<Object> consultarOrdenes(Paginado paginado, Integer idContratante, Authentication authentication) throws IOException {
		...
		try {
			...
			if (result.isEmpty()) {
				return new Response<>(false, HttpStatus.OK.value(), AppConstantes.HISTORIAL_ORDENES_MENSAJE, Arrays.asList());
			}
		} catch (Exception e) {
	...
	public Response<Object> consultarVerDetalleOrdenes(Integer idOrden, Authentication authentication)
		...
		try(SqlSession sqlSession=sqlSessionFactory.openSession()) {
			...
			consultaHistorialDetalle=consultas.selectNativeQuery(historialOrdenes.consultarVerDetalleHistorialServicio(idOrden));
			
			detalleResponse=Arrays.asList(mapper.map(consultaVerDetalle, DetalleResponse[].class));
			detalleHistorialServicios=Arrays.asList(mapper.map(consultaHistorialDetalle, DetalleHistorialServicios[].class));
