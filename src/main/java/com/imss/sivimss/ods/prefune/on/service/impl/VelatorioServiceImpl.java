...
public class VelatorioServiceImpl implements VelatorioService, CatalogosService {
	...
	private static final String CODIGO_POSTAL_NO_EXISTE = "184"; // El codigo postal no existe.
	...
	public Response<Object> consultarServiciosVelatorios(Authentication authentication) throws IOException {
		...
		try (SqlSession sqlSession = sqlSessionFactory.openSession()) {
			...
			velatoriosResponses.stream().forEach(registro -> {
				resultServiciosVelatorios = consultas
						.selectNativeQuery(mapaVelatorio.consultarServiciosVelatorios(registro.getIdVelatorio()));
	 ...
	 */
	@Override
	public Response<Object> consultarCatalogoPaquete(Integer idVelatorio, Authentication authentication)
			throws IOException {
		...
		try (SqlSession sqlSession = sqlSessionFactory.openSession()) {
			...
			paqueteResponses.stream().forEach(registro -> {
				resultServiciosVelatorios = consultas
						.selectNativeQuery(catalogos.obtenerCaracteristicasPaquete(registro.getIdPaquete()));
	...
	public Response<Object> consultarCodigoPostal(String codigoPostal, Authentication authentication)
		...
		try {
			Response<Object> response = providerRestTemplate
					.consumirServicioExternoGet(urlSepomex + "/" + codigoPostal);
			return MensajeResponseUtil.mensajeResponseExterno(response, CODIGO_POSTAL_NO_EXISTE,
					SERVICIO_SEPOMEX_NO_DISPONIBLE);
		} catch (Exception e) {
	...
	@Override
	public Response<Object> consultarEstado(Authentication authentication) throws IOException {
		SqlSessionFactory sqlSessionFactory = myBatisConfig.buildqlSessionFactory();
		try (SqlSession sqlSession = sqlSessionFactory.openSession()) {
			Consultas consultas = sqlSession.getMapper(Consultas.class);
			resultServiciosCatalogo = consultas.selectNativeQuery(catalogos.consultarEstado());
